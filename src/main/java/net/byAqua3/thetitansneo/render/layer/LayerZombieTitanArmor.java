...
@OnlyIn(Dist.CLIENT)
public class LayerZombieTitanArmor extends RenderLayer<TitanRenderState, ModelZombieTitan> {
	...
	public LayerZombieTitanArmor(RenderLayerParent<TitanRenderState, ModelZombieTitan> renderer) {
		super(renderer);
	}
	...
	protected int color(TitanRenderState state, float f) {
		return new Color(0.0F, 0.6F + Mth.cos(this.xOffset(f) * 0.05F) * 0.2F, 0.0F, 1.0F).getRGB();
	}
