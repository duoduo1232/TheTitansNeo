...
@OnlyIn(Dist.CLIENT)
public class LayerBlazeTitanArmor extends RenderLayer<TitanRenderState, ModelBlazeTitan> {
	...
	public LayerBlazeTitanArmor(RenderLayerParent<TitanRenderState, ModelBlazeTitan> renderer) {
		super(renderer);
	}
	...
	protected int color(TitanRenderState state, float f) {
		return new Color(0.5F + Mth.cos(f * 0.3F) * 0.2F, 0.3F + Mth.cos(f * 0.3F) * 0.2F, 0.0F, 1.0F).getRGB();
	}
