...
public class EntityWitherzillaMinion extends WitherBoss implements IMinion {
	...
	public boolean canAttack(LivingEntity target) {
		...
		return !target.is(TheTitansNeoEntities.WITHERZILLA.get()) && !target.is(TheTitansNeoEntities.WITHER_SKELETON_TITAN_MINION.get()) && !target.is(TheTitansNeoEntities.WITHER_TURRET.get()) && !target.is(TheTitansNeoEntities.WITHER_TURRET_GROUND.get()) && !target.is(TheTitansNeoEntities.WITHER_TURRET_MORTAR.get()) && target.canBeSeenByAnyone() && this.canAttackEntity(target);
	}
