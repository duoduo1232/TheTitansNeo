...
public class EntityZombifiedPiglinTitanMinion extends ZombifiedPiglin implements RangedAttackMob, IMinion {
	...
	private static final EntityDataAccessor<Integer> MINION_TYPE = SynchedEntityData.defineId(EntityZombifiedPiglinTitanMinion.class, EntityDataSerializers.INT);
	...
	public EntityZombifiedPiglinTitanMinion(EntityType<? extends EntityZombifiedPiglinTitanMinion> entityType, Level level) {
	...
	public EntityZombifiedPiglinTitanMinion(Level level) {
		this(TheTitansNeoEntities.ZOMBIFIED_PIGLIN_TITAN_MINION.get(), level);
	}

	public static AttributeSupplier.Builder createAttributes() {
		return ZombifiedPiglin.createAttributes().add(Attributes.FOLLOW_RANGE, 32.0D).add(Attributes.MOVEMENT_SPEED, 0.25F).add(Attributes.EXPLOSION_KNOCKBACK_RESISTANCE, 1.0D);
	}
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(0, new EntityAIFindMaster(this, 100.0D, titan -> titan instanceof EntityZombifiedPiglinTitan));
	...
	public Component getName() {
		...
		case PRIEST:
			return Component.translatable("entity.thetitansneo.zombified_piglin_priest");
		case ZEALOT:
			return Component.translatable("entity.thetitansneo.zombified_piglin_zealot");
		case BISHOP:
			return Component.translatable("entity.thetitansneo.zombified_piglin_bishop");
		case TEMPLAR:
			return Component.translatable("entity.thetitansneo.zombified_piglin_templar");
		default:
			return Component.translatable("entity.thetitansneo.zombified_piglin_loyalist");
		}
	}
	...
	public boolean hurtServer(ServerLevel level, DamageSource damageSource, float amount) {
		...
		if (entity instanceof EntityZombifiedPiglinTitanMinion || entity instanceof EntityZombifiedPiglinTitan) {
			return false;
		}
	...
	public void performRangedAttack(LivingEntity target, float velocity) {
		...
		if (this.distanceToSqr(target) < (target.getBbWidth() * target.getBbWidth()) + 36.0D) {
			...
			if (randomInt == 0) {
				...
				this.playSound(SoundEvents.ZOMBIFIED_PIGLIN_HURT, 1.0F, 0.5F);
				...
				for (int i = 0; i < 128; i++) {
					...
					ItemEntity itemEntity1 = new ItemEntity(this.level(), target.getX(), target.getY(), target.getZ(), new ItemStack(Items.GOLD_NUGGET));
					itemEntity1.setPickUpDelay(6000);
					itemEntity1.lifespan = 40 + this.getRandom().nextInt(20);
					itemEntity1.push(0.0D, 1.0D, 0.0D);
					if (!this.level().isClientSide()) {
			...
			if (this.getMaster() == null && this.isAlive()) {
				if (!this.level().isClientSide() && this.level().getDifficulty() != Difficulty.PEACEFUL) {
					...
					if (randomRate < TheTitansNeoConfigs.bishopSummonMinionSpawnRate.get()) {
						...
						this.level().addFreshEntity(zombifiedPiglinTitanMinion);
					}
