...
import net.byAqua3.thetitansneo.entity.ai.minion.blaze.EntityAIBlazeTitanMinionAttack;
...
public class EntityBlazeTitanMinion extends Blaze implements RangedAttackMob, IMinion {
	...
	private static final EntityDataAccessor<Integer> MINION_TYPE = SynchedEntityData.defineId(EntityBlazeTitanMinion.class, EntityDataSerializers.INT);

	public final RangedAttackGoal rangedAttackGoal = new RangedAttackGoal(this, 1.0D, 1, 64.0F);
	...
	public EntityBlazeTitanMinion(EntityType<? extends EntityBlazeTitanMinion> entityType, Level level) {
	...
	public EntityBlazeTitanMinion(Level level) {
		this(TheTitansNeoEntities.BLAZE_TITAN_MINION.get(), level);
	}

	public static AttributeSupplier.Builder createAttributes() {
		return Blaze.createAttributes().add(Attributes.FOLLOW_RANGE, 24.0D).add(Attributes.MOVEMENT_SPEED, 0.25D).add(Attributes.EXPLOSION_KNOCKBACK_RESISTANCE, 1.0D);
	}
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(0, new EntityAIFindMaster(this, 100.0D, titan -> titan instanceof EntityBlazeTitan));
		...
		this.goalSelector.addGoal(0, new EntityAIBlazeTitanMinionAttack(this));
	...
	public Component getName() {
		...
		case PRIEST:
			return Component.translatable("entity.thetitansneo.blaze_priest");
		case ZEALOT:
			return Component.translatable("entity.thetitansneo.blaze_zealot");
		case BISHOP:
			return Component.translatable("entity.thetitansneo.blaze_bishop");
		case TEMPLAR:
			return Component.translatable("entity.thetitansneo.blaze_templar");
		default:
			return Component.translatable("entity.thetitansneo.blaze_loyalist");
		}
	}
	...
	public void refreshAttributes() {
		...
		case BISHOP:
			...
			break;
		case TEMPLAR:
			this.getAttribute(Attributes.MAX_HEALTH).setBaseValue(2000.0D);
			this.getAttribute(Attributes.ATTACK_DAMAGE).setBaseValue(60.0D);
	...
	public boolean canAttack(LivingEntity target) {
		...
		return !target.is(TheTitansNeoEntities.BLAZE_TITAN.get()) && !target.is(TheTitansNeoEntities.BLAZE_TITAN_MINION.get()) && target.canBeSeenByAnyone() && this.canAttackEntity(target);
	}
	...
	public boolean hurtServer(ServerLevel level, DamageSource damageSource, float amount) {
		...
		if (entity instanceof EntityBlazeTitanMinion || entity instanceof EntityBlazeTitan) {
			return false;
		}
		...
		if (entity instanceof LivingEntity) {
			...
			if (this.canAttack(livingEntity)) {
				...
				for (Entity minionEntity : entities) {
					if (minionEntity instanceof EntityBlazeTitanMinion) {
						EntityBlazeTitanMinion blazeTitanMinion = (EntityBlazeTitanMinion) minionEntity;
						blazeTitanMinion.setTarget(livingEntity);
						blazeTitanMinion.randomSoundDelay = this.getRandom().nextInt(40);
					}
		...
		if (this.distanceToSqr(target) < (target.getBbWidth() * target.getBbWidth()) + 36.0D) {
			...
			if (randomInt == 0) {
				...
				for (int i = 0; i < 50; i++) {
					...
					EntityBlazeTitanMinionSmallFireball smallFireball = new EntityBlazeTitanMinionSmallFireball(this.level(), this, new Vec3(d0 + this.getRandom().nextGaussian() * d3, d1, d2 + this.getRandom().nextGaussian() * d3));
